  ...
  persist(
    (set, get) => ({
      ...
      fetchWeatherMood: async () => {
        try {
          const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/weather`);
      ...
      fetchMusic: async (mood) => {
        try {
         const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/music?mood=${mood}`);
